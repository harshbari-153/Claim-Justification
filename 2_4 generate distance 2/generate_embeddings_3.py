...
def get_sentence_distance(sentence):
  ...
  i = 0
  while i < n and subject == False:
    if (pos_tags[i][1][:2] == "PR" or pos_tags[i][1][-2:] == "DT"):
      main_subject = pos_tags[i][0]
      subject = True
    i += 1
  ...
  while i < n and action == False:
    if (pos_tags[i][1][:2] == "JJ" or pos_tags[i][1][:2] == "VB"):
  ...
  return np.array(sim_model.encode(main_subject, convert_to_tensor=True)) - np.array(sim_model.encode(main_action, convert_to_tensor=True))
#################################################
...
def distance_vector(claim, justifications, top_n_indices):
  ...
  j_distance = np.array([0 for x in range(384)])
